...
================================================================================
Generate professional PDF and Excel reports for neuromarketing analysis.
...
class ReportGenerator:
    ...
    def generate_pdf_report(self, heatmap_paths=None, trajectory_paths=None):
        ...
        if heatmap_paths:
            ...
            print(f"Adding {len(heatmap_paths)} heatmaps to PDF...")
            for heatmap in heatmap_paths:
                # Use display_name with consistent format
                scene_idx = heatmap.get('scene_index', 0)
                custom_name = heatmap.get('scene_custom_name') or heatmap.get('scene_name')
                display_name = heatmap.get('display_name') or f"Heatmap - Scene {scene_idx} - {custom_name}"
                story.append(Paragraph(f"{display_name}", styles['Heading3']))
                
                # Verify file exists and log
                heatmap_path = heatmap['path']
                if os.path.exists(heatmap_path):
                    print(f"  ✓ Adding heatmap: {os.path.basename(heatmap_path)}")
                    try:
                        # Get image size to maintain aspect ratio
                        from PIL import Image as PILImage
                        pil_img = PILImage.open(heatmap_path)
                        img_width, img_height = pil_img.size
                        aspect_ratio = img_height / img_width
                        
                        # Set display size (max 6 inches wide)
                        display_width = 6 * inch
                        display_height = display_width * aspect_ratio
                        
                        img = Image(heatmap_path, width=display_width, height=display_height)
                        story.append(img)
                        story.append(Spacer(1, 0.3*inch))
                    except Exception as e:
                        print(f"  ✗ Error adding heatmap image: {e}")
                        story.append(Paragraph(f"<i>Error loading image: {e}</i>", styles['Normal']))
                else:
                    print(f"  ✗ Heatmap file not found: {heatmap_path}")
                    story.append(Paragraph(f"<i>Image file not found: {os.path.basename(heatmap_path)}</i>", styles['Normal']))
                    
            story.append(PageBreak())
        else:
            print("No heatmap paths provided to PDF generator")
        ...
        if trajectory_paths:
            ...
            print(f"Adding {len(trajectory_paths)} trajectory images to PDF...")
            for trajectory in trajectory_paths:
                # Use display_name with consistent format
                scene_idx = trajectory.get('scene_index', 0)
                custom_name = trajectory.get('scene_custom_name') or trajectory.get('scene_name')
                display_name = trajectory.get('display_name') or f"Gaze Trajectory - Scene {scene_idx} - {custom_name}"
                story.append(Paragraph(f"{display_name}", styles['Heading3']))
                ...
                if os.path.exists(trajectory_path):
                    print(f"  ✓ Adding trajectory: {os.path.basename(trajectory_path)}")
                    try:
                        # Get image size to maintain aspect ratio
                        from PIL import Image as PILImage
                        pil_img = PILImage.open(trajectory_path)
                        img_width, img_height = pil_img.size
                        aspect_ratio = img_height / img_width
                        ...
                        img = Image(trajectory_path, width=display_width, height=display_height)
                        story.append(img)
                        story.append(Spacer(1, 0.3*inch))
                    except Exception as e:
                        print(f"  ✗ Error adding trajectory image: {e}")
                        story.append(Paragraph(f"<i>Error loading image: {e}</i>", styles['Normal']))
                else:
                    print(f"  ✗ Trajectory file not found: {trajectory_path}")
                    story.append(Paragraph(f"<i>Image file not found: {os.path.basename(trajectory_path)}</i>", styles['Normal']))
        else:
            print("No trajectory paths provided to PDF generator")
