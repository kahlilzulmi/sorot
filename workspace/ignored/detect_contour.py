...
### FUNGSI BARU: Deteksi menggunakan kontur ###
def detect_gaze_from_contour(gray_img, min_area=50, threshold_value=100):
    """
    Detects the gaze point by finding the largest contour in the image.
    This is robust for outline-style shapes.
    """
    # 1. Thresholding untuk mengisolasi outline
    _, thresh = cv2.threshold(gray_img, threshold_value, 255, cv2.THRESH_BINARY)
    
    # 2. Cari semua kontur pada gambar hasil threshold
    contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    if not contours:
        return None, None # Tidak ada kontur yang ditemukan

    # 3. Filter kontur dan pilih yang terbesar
    largest_contour = max(contours, key=cv2.contourArea)
    
    # Pastikan kontur terbesar melebihi area minimum untuk menghindari noise
    if cv2.contourArea(largest_contour) < min_area:
        return None, None

    # 4. Hitung titik tengah (sentroid) dari kontur terbesar
...
while True:
    ...
    ### PERUBAHAN UTAMA: Panggil fungsi deteksi kontur ###
    cx, cy = detect_gaze_from_contour(gray)
    
    overlay_frame = frame.copy()
    
    # Gambar crosshair jika titik ditemukan
    if cx is not None and cy is not None:
        draw_crosshair(overlay_frame, cx, cy, (0, 255, 0), label='Detected')

    # Anotasi frame (selalu, meskipun titik tidak ditemukan)
...
# Hapus baris di mana deteksi gagal, jika diperlukan
