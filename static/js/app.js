...
createApp({
    data() {
        return {
            ...
            // Import Mode Data
            importMode: 'auto',  // 'auto' or 'csv'
            importedGazeData: null,
            importedGazeFile: null,
            importedVideoFile: null,
            importedGazeVideoFile: null,  // Optional Tobii Ghost eye gaze video
            ...
            showExportDropdown: false,  // Dropdown for export format selection
            exportFormat: 'csv',  // Export format: 'csv' or 'json'
            exportFilename: '',  // Custom filename for export
            exportInProgress: false,  // Export processing flag
            ...
            progressPollCount: 0,
            maxProgressPolls: 600, // 10 minutes at 1 poll/second
            manualImportRequired: false,
            obsRecordingPath: null,
            importingOBSFile: false,
            showOBSFilePickerModal: false,
            availableOBSFiles: [],
            loadingOBSFiles: false,
    ...
    computed: {
        ...
        totalROIs() {
            // Count total ROIs across all scenes
            return this.scenes.reduce((total, scene) => total + (scene.rois?.length || 0), 0);
        }
    },
    ...
    mounted() {
        ...
        // Prevent Escape key from exiting fullscreen during recording
        ...
        // Re-enter fullscreen if exited during active recording
        document.addEventListener('fullscreenchange', () => {
            if (!document.fullscreenElement && this.recording) {
    ...
    methods: {
        ...
        selectMode(mode) {
            if (mode === 'live') {
                ...
                this.appMode = 'import';
                this.importMode = 'auto';  // Default to auto-detect mode
        ...
        selectAutoGazeVideo(event) {
            /**
             * Select eye gaze video for auto-detection mode.
             */
            const file = event.target.files[0];
            if (!file) return;
        ...
        async processAutoDetect() {
            /**
             * Process eye gaze video using auto-detection.
             * Upload both videos and trigger backend processing.
             */
            if (!this.importedGazeVideoFile || !this.importedVideoFile) {
                alert('Please select both eye gaze video and stimulus video');
                return;
            }
            
            this.showImportModal = false;
            this.statusMessage = 'Uploading videos for auto-detection...';
            
            try {
                // Upload stimulus video first
                const stimulusData = new FormData();
                stimulusData.append('video', this.importedVideoFile);
                
                const stimulusResponse = await axios.post('/upload-video', stimulusData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                
                if (stimulusResponse.data.error) {
                    throw new Error(stimulusResponse.data.error);
                }
                
                this.videoInfo = stimulusResponse.data;
                this.videoSrc = `/video/${this.videoInfo.filename}`;
                this.videoDuration = this.videoInfo.duration;
                
                // Upload gaze video
                const gazeData = new FormData();
                gazeData.append('video', this.importedGazeVideoFile);
                
                const gazeResponse = await axios.post('/upload-video', gazeData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                
                if (gazeResponse.data.error) {
                    throw new Error(gazeResponse.data.error);
                }
                
                // Set up dual video mode
                this.gazeVideoSrc = `/video/${gazeResponse.data.filename}`;
                this.videoMode = 'dual';
                
                // Initialize with single scene spanning entire video
                this.scenes = [{
                    name: 'Scene 1',
                    custom_name: '',
                    start_frame: 0,
                ...
                this.statusMessage = 'Videos loaded. Processing gaze coordinates...';
                
                // Start auto-detection in background
                this.startAutoGazeDetection(gazeResponse.data.filename);
                
            } catch (error) {
                console.error('Auto-detect error:', error);
                alert(`Error: ${error.message}`);
                this.statusMessage = 'Auto-detection failed';
            }
        },
        
        async startAutoGazeDetection(gazeVideoFilename) {
            /**
             * Call backend to process gaze video and extract coordinates.
             */
            try {
                this.statusMessage = 'Auto-detecting gaze coordinates from video...';
                
                const response = await axios.post('/api/import/auto-detect-gaze', {
                    gaze_video_filename: gazeVideoFilename,
                    stimulus_video_info: this.videoInfo
                });
                
                if (response.data.success) {
                    this.importedGazeData = response.data.gaze_data;
                    this.statusMessage = `Auto-detection complete! Detected ${response.data.total_points} gaze points (${response.data.detection_rate.toFixed(1)}% success rate)`;
                    
                    // Show success notification
                    alert(`Auto-detection complete!\\n\\nTotal frames: ${response.data.total_frames}\\nDetected: ${response.data.total_points} points\\nSuccess rate: ${response.data.detection_rate.toFixed(1)}%`);
                } else {
                    throw new Error(response.data.error || 'Auto-detection failed');
                }
            } catch (error) {
                console.error('Auto-detection error:', error);
                alert(`Auto-detection error: ${error.message}`);
                this.statusMessage = 'Auto-detection failed - you can still define ROIs manually';
            }
        },
        ...
        async exportScenesROIs(format = 'csv', customFilename = null) {
            ...
            if (!this.videoInfo || !this.scenes || this.scenes.length === 0) {
                this.statusMessage = 'No scenes to export. Please define scenes first.';
                return;
            }
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    const message = format === 'csv' 
                        ? `✅ Exported ${result.rows} rows to ${result.filename} | Scenes: ${result.scenes}, ROIs: ${result.total_rois}`
                        : `✅ Exported to ${result.filename} | Scenes: ${result.scenes}, ROIs: ${result.total_rois}`;
                    
                    this.statusMessage = message;
                    
                    // Log success for user reference
                ...
                const errorMsg = error.response?.data?.error || error.message;
                this.statusMessage = '❌ Export failed: ' + errorMsg;
                this.exportInProgress = false; // Reset flag on error
        ...
        toggleExportDropdown() {
            /**
             * Toggle the export format dropdown menu.
             */
            this.showExportDropdown = !this.showExportDropdown;
        },
        
        async quickExport(format = 'csv') {
            /**
             * Direct export without modal - exports immediately with auto-generated filename.
             * @param {string} format - Export format: 'csv' or 'json'
             */
            // Close dropdown
            ...
            this.statusMessage = `Exporting to ${format.toUpperCase()}...`;
            
            // Trigger export with auto-generated filename
            await this.exportScenesROIs(format, null);
        },
        ...
        async runPostProcessingPipeline() {
            ...
            this.progressMessage = 'Starting post-processing...';
            
            try {
                // Start post-processing (runs in background thread)
                const response = await axios.post('/api/processing/start-post-processing');
                ...
                this.progressMessage = 'Failed to start processing';
        ...
        async pollProgress() {
            // Safety check for maximum polls
            if (this.progressPollCount >= this.maxProgressPolls) {
                console.warn('Maximum poll count reached');
                this.stopProgressPolling();
                this.progressError = 'Processing timeout - took longer than expected';
                this.processing = false;
                return;
            }
            ...
            try {
                const response = await axios.get('/api/processing/progress');
                const data = response.data;
                
                // Update progress
                this.progressPercent = data.progress || 0;
                this.progressMessage = data.status || 'Processing...';
                
                // Check for completion
                if (data.complete && data.progress >= 100) {
                    console.log('Processing marked complete, verifying files...');
                ...
                if (data.error || data.progress < 0) {
                    this.stopProgressPolling();
                    this.progressError = data.status || 'Processing failed';
                    this.progressMessage = 'Processing encountered an error';
                    this.processing = false;
                    return;
                }
                
                // Continue polling if still processing
                if (!data.is_processing && !data.complete) {
                    // Processing stopped but not complete - might be an issue
                    console.warn('Processing stopped but not marked complete');
                    await this.verifyProcessingCompletion();
                }
        ...
        async verifyProcessingCompletion() {
            try {
                const response = await axios.get('/api/processing/verify-completion');
                const data = response.data;
                
                if (data.complete) {
                    console.log('Processing verified complete!', data.files_found);
                    this.generatedFiles = data.files_found || {};
                    this.progressComplete = true;
                    this.progressPercent = 100;
                    this.progressMessage = 'Processing complete!';
                    this.statusMessage = 'Processing complete!';
                    
                    // Update Lucide icons after files are rendered
                    this.$nextTick(() => {
                        if (window.lucide) {
                            lucide.createIcons();
                        }
                    ...
                    console.warn('Processing marked complete but files missing:', data);
                    this.progressError = 'Some output files may be missing';
                    this.progressMessage = 'Processing completed with warnings';
                }
                
            } catch (error) {
                console.error('Error verifying completion:', error);
                this.progressError = 'Could not verify processing results';
            } finally {
                this.stopProgressPolling();
                this.processing = false;
            }
        ...
        async startRecording() {
            ...
            // Prepare UI for recording (OBS will start when user clicks START button)
            try {
                // Recording UI setup
                this.recording = true;
                this.framesRecorded = 0;
                this.sessionDir = null;  // Will be set when START is clicked
                
                // Update status message
                this.statusMessage = 'Ready to record - Click START to begin';
                console.log('✓ Recording UI prepared - waiting for START button');
        ...
        async startPlaybackAndRecording() {
            // Start OBS recording when START button is clicked
            try {
                ...
                console.error('Recording start error:', error);
                const errorMsg = error.response?.data?.error || error.message || 'Unknown error';
                alert('Failed to start OBS recording: ' + errorMsg);
        ...
        async stopRecording() {
            ...
            try {
                ...
                // Check if manual import is required
                if (response.data.manual_import_required) {
                    this.manualImportRequired = true;
                    this.obsRecordingPath = null;
                    alert(`Recording timestamps saved!\nFrames: ${response.data.timestamps_recorded}\n⚠️ Eye gaze video file not found automatically.\n\nPlease use the Post-process button and manually select the eye gaze recording file.`);
                } else {
                    this.manualImportRequired = false;
                    this.obsRecordingPath = response.data.obs_recording_path;
                    await this.runPostProcessingPipeline();
                    this.progressMessage = 'Recording complete!';
        ...
        async importOBSRecordingFile() {
            // Load available OBS files and show picker modal
            this.loadingOBSFiles = true;
            this.showOBSFilePickerModal = true;
            
            try {
                const response = await axios.get('/api/settings/list-obs-recordings');
                if (response.data.success) {
                    this.availableOBSFiles = response.data.files;
                } else {
                    alert('Failed to load available files: ' + response.data.error);
                    this.showOBSFilePickerModal = false;
                }
            } catch (error) {
                alert('Error loading files: ' + error.message);
                this.showOBSFilePickerModal = false;
            } finally {
                this.loadingOBSFiles = false;
            }
        },
        
        async selectOBSFile(filePath) {
            this.importingOBSFile = true;
            try {
                const response = await axios.post('/api/settings/import-obs-recording', {
                    file_path: filePath
                });
                
                if (response.data.success) {
                    this.obsRecordingPath = response.data.destination;
                    this.manualImportRequired = false;
                    this.showOBSFilePickerModal = false;
                    alert(`✅ Eye gaze file imported successfully!\n${response.data.filename}\n\nYou can now proceed with post-processing.`);
                } else {
                    alert(`❌ Failed to import file: ${response.data.error}`);
                }
            } catch (error) {
                alert(`Error importing file: ${error.message}`);
            } finally {
                this.importingOBSFile = false;
            }
        },
        ...
        async processAll() {
            ...
            if (this.manualImportRequired) {
                alert('⚠️ Please import the eye gaze recording file first using the "Import Eye Gaze File" button.');
                return;
            }
